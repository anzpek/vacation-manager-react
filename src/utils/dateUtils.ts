// dateUtils.ts - 날짜 관련 유틸리티 함수들 (TypeScript 버전)
...
// Date input type (Date object or string)
...
export const formatDateToYYYYMMDD = (date: DateInput): string => {
...
// 날짜를 한국어 형식으로 포맷팅
export const formatDateToKorean = (date: DateInput | null | undefined): string => {
    if (!date) return '';
...
// 같은 월인지 확인
export const isSameMonth = (date: DateInput, year: number, month: number): boolean => {
    if (typeof date === 'string') {
        date = new Date(date);
    }
...
// 연속된 날짜인지 확인
export const isConsecutiveDay = (date1: DateInput, date2: DateInput): boolean => {
...
// 공휴일인지 확인
export const isHoliday = (date: DateInput): boolean => {
    const dateString = formatDateToYYYYMMDD(date);
    return !!HOLIDAYS_2025[dateString];
};

// 공휴일 이름 반환
export const getHolidayName = (date: DateInput): string | null => {
    const dateString = formatDateToYYYYMMDD(date);
    return HOLIDAYS_2025[dateString] || null;
};

// 휴일(주말 + 공휴일)인지 확인
export const isRestDay = (date: DateInput): boolean => {
    return isWeekendDay(date) || isHoliday(date);
};

// 영업일인지 확인
export const isWorkingDay = (date: DateInput): boolean => {
    return !isRestDay(date);
};

// 다음 영업일 찾기
export const getNextWorkingDay = (date: DateInput): Date => {
...
// 이전 영업일 찾기
export const getPreviousWorkingDay = (date: DateInput): Date => {
    if (typeof date === 'string') date = new Date(date);
...
// 두 날짜 사이의 영업일 수 계산
export const getWorkingDaysBetween = (startDate: DateInput, endDate: DateInput): number => {

...
import { formatDateToKorean } from '../../utils/dateUtils.ts';
...
const VacationModal = () => {
    ...
    useEffect(() => {
        if (ui.activeModal === 'vacation' || ui.activeModal === 'editVacation') {
            if (mountedRef.current) {
                console.warn('[VacationModal] 🚨 중복 마운트 시도 차단');
                return;
            }
    ...
    // 디버그 로그
    console.log('[VacationModal] 렌더링 체크:', { 
    ...
    // 모달이 열릴 때 데이터 초기화
    useEffect(() => {
        console.log('[VacationModal] 🔄 데이터 초기화 시작:', {
            activeModal: ui.activeModal,
            modalProps: ui.modalProps,
            hasVacation: !!ui.modalProps?.vacation,
            isEditMode: ui.activeModal === 'editVacation'
        });
        ...
        if (ui.activeModal === 'vacation' || ui.activeModal === 'editVacation') {
            ...
            } else if (ui.modalProps && ui.modalProps.date) {
                console.log('[VacationModal] 📅 modalProps에서 date 사용:', ui.modalProps);
        ...
        setErrors({});
    }, [ui.activeModal, ui.selectedDate, ui.modalProps?.vacation?.id, ui.modalProps?.date, ui.modalProps?.employee?.id]);
    ...
    }, [formData.date, formData.employeeId, actions, formData.id, isEditMode, ui.modalProps?.consecutiveGroup]);
    ...
    if (ui.activeModal !== 'vacation' && ui.activeModal !== 'editVacation') {
        console.log('[VacationModal] ❌ 모달 렌더링 안 함 - activeModal:', ui.activeModal);
        return null;
    }

    console.log('[VacationModal] 🎨 모달 렌더링 시작:', {
        activeModal: ui.activeModal,
        isEditMode,
        formData,
    ...
    return (
        <div className="vacation-modal-overlay" onClick={handleClose}>
            <div className="vacation-modal-container" onClick={(e) => e.stopPropagation()}>
                <div className="vacation-modal-header">
                    <h2 className="vacation-modal-title">{getModalTitle()}</h2>
                    <button className="vacation-modal-close" onClick={handleClose}>
                    ...
                    if (isConsecutiveVacation) {
                        ...
                        return (
                            <div className="edit-context-info">
                                <div className="context-details">
                                    <div className="context-item">
                                        <span className="context-label">수정중인 휴가</span>
                                        <span className="context-label">현재 유형:</span>
                                        <span className="context-value">{formData.type}</span>
                                    </div>
                                </div>
...
export default memo(VacationModal);